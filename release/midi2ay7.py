...
class MidiToAY:
    ...
    def generate_asm(self, org=0xB000, loop=False):
        """Generate Z80 assembly code.

        loop: if True, replaying resumes from the start automatically when
              the data ends, instead of exiting. Either way, pressing any
              key always triggers a clean exit back to BASIC.
        """
        asm = [
            f"\torg\t0{org:04X}h",
            "",
        ...
        if loop:
            asm += [
                f"\tld\thl,note_data_src",
                "\tld\tde,note_data",
                f"\tld\tbc,{len(self.output)}",
                "\tldir",
            ]
        asm += [
            "\tld\thl,0FEFFh",
            ...
            "main_loop:",
            "\thalt",
            "\tld\tbc,00FEh",      # select all keyboard half-rows at once
            "\tin\ta,(c)",
            "\tand\t1Fh",
            "\tcp\t1Fh",
            "\tjr\tnz,do_exit",     # any key pressed -> clean exit
            "\tld\ta,(music_playing)",
            "\tor\ta",
            "\tjr\tnz,main_loop",   # still playing -> keep waiting for frames
        ]
        if loop:
            asm += [
                # the ISR decrements wait-bytes in place as it plays, so the
                # live buffer must be refreshed from the pristine copy
                # before every replay -- just resetting the pointer isn't
                # enough, the data itself has been consumed.
                f"\tld\thl,note_data_src",
                "\tld\tde,note_data",
                f"\tld\tbc,{len(self.output)}",
                "\tldir",
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python midi2ay.py <input.mid> [output.asm] [--loop]")
        print("  --loop  restart automatically when the song ends")
        print("          (default: play once and exit). Either way,")
        print("          pressing any key always exits cleanly.")
    ...
    input_file = args[0]
    output_file = args[1] if len(args) > 1 else input_file.rsplit('.', 1)[0] + '.asm'

    converter = MidiToAY()
    converter.parse_midi_file(input_file)

    with open(output_file, 'w') as f:
        f.write(converter.generate_asm(loop=loop))
